<template>
    <div class="vm-mask-black-dialog">
        <div class="vm-confirm vm-alert">
            <div class="vm-confirm-bd" v-html="mes"></div>
            <div class="vm-confirm-ft">
                <a href="javascript:;" class="vm-confirm-btn primary" @click.stop="closeAlert">确定</a>
            </div>
        </div>
    </div>
</template>

<script>
export default {
    props: {
        mes: String,
        callback: Function
    }
}
</script>

<style lang="less">
@import "../../../../styles/variables";
@import "../../../../styles/mixins";

@vm-icon-success: 'data:image/png;base64,';
@vm-icon-error: 'data:image/png;base64,';
@icon-loading: 'data:image/png;base64,';

.vm-mask-black-dialog {
    .mask(rgba(0, 0, 0, .4), @base-zindex * 1500);
}

.vm-mask-white-dialog {
    .mask(rgba(0, 0, 0, 0), @base-zindex * 1600);
}

@keyframes zoomIn {
    from {
        opacity: 0;
        transform: scale3d(.3, .3, .3);
    }
    50% {
        opacity: 1;
    }
}

// Confirm & Alert =================
.vm-confirm {
    width: 85%;
    background-color: #FAFAFA;
    border-radius: 2px;
    font-size: 15px;
    animation: zoomIn .15s ease forwards;
}

.vm-alert {
    animation: zoomIn .15s ease forwards;
    .vm-confirm-bd {
        text-align: center;
        padding: 20px 20px 0 20px;
    }
    .vm-confirm-ft {
        margin-top: 14px;
    }
}

.vm-confirm-hd {
    text-align: left;
    padding: 15px 20px 5px;
    .vm-confirm-title {
        font-weight: normal;
        color: #444;
        word-break: break-all;
    }
}

.vm-confirm-bd {
    text-align: left;
    padding: 0 20px;
    font-size: 14px;
    color: #888;
    line-height: 20px;
    word-break: break-all;
}

.vm-confirm-ft {
    position: relative;
    line-height: 40px;
    margin-top: 14px;
    display: flex;
    &:after {
        .top-line(@line-color);
    }
    > a {
        position: relative;
        text-align: center;
        display: block;
        flex: 1;
        padding: 0 2px;
        &:not(:last-child):after {
            .right-line(@line-color);
        }
        &.vm-confirm-btn.default {
            color: #353535;
        }
        &.vm-confirm-btn.primary {
            color: #0BB20C;
        }
    }
}

@media screen and (min-width: 768px) {
    .vm-confirm {
        width: 40%;
    }
}

// Toast =================
.vm-toast {
    min-width: 130px;
    max-width: 80%;
    padding-top: 20px;
    background: rgba(40, 40, 40, .8);
    text-align: center;
    border-radius: 3px;
    color: #FFF;
    animation: zoomIn .06s ease forwards;
    &.vm-toast-none-icon {
        padding-top: 10px;
        border-radius: 3px;
        .vm-toast-content {
            padding: 0 36px 10px 36px;
        }
    }
}

.vm-toast-content {
    font-size: 15px;
    padding: 0 15px 15px 15px;
    line-height: 22px;
    word-break: break-all;
}

.vm-toast-success-icon, .vm-toast-error-icon {
    display: block;
    margin-bottom: 10px;
    &:after {
        display: inline-block;
        content: '';
    }
}

.vm-toast-success-icon:after {
    width: 43px;
    height: 35px;
    background: url(@vm-icon-success) no-repeat;
    background-size: 43px 35px;
}

.vm-toast-error-icon:after {
    width: 35px;
    height: 35px;
    background: url(@vm-icon-error) no-repeat;
    background-size: 35px 35px;
}

// Notify =================
@keyframes notify-downin {
    0% {
        opacity: 0;
        transform: translate3d(0, -50px, 0);
    }
    50% {
        opacity: .5;
    }
    100% {
        opacity: 1;
        transform: translate3d(0, 0, 0);
    }
}

@keyframes notify-upout {
    0% {
        opacity: 1;
        transform: translate3d(0, 0, 0);
    }
    50% {
        opacity: 1;
    }
    100% {
        opacity: 0;
        transform: translate3d(0, -50px, 0);
    }
}

.vm-notify {
    position: fixed;
    top: 0;
    left: 0;
    width: 100%;
    background-color: rgba(40, 40, 40, .8);
    line-height: .28rem*@suiremscale;
    font-size: .26rem*@suiremscale;
    color: #FFF;
    padding: .3rem*@suiremscale .24rem*@suiremscale;
    opacity: 0;
    animation: notify-downin .2s linear forwards;
    word-break: break-all;
    text-align: center;
    &.notify-out {
        opacity: 1;
        animation: notify-upout .15s linear forwards;
    }
}

// Loading =================
@keyframes rotate-loading {
    0% {
        transform: rotate(0);
    }
    100% {
        transform: rotate(360deg);
    }
}

.vm-loading {
    border-radius: 5px;
    color: #FFF;
    background-color: rgba(40, 40, 40, .8);
    animation: zoomIn .1s ease forwards;
    display: flex;
    align-items: center;
    padding: 0 23px 0 24px;
    height: 48px;
    .vm-loading-icon {
        width: 28px;
        height: 28px;
        background: url(@icon-loading) no-repeat;
        background-size: 28px 28px;
        animation: rotate-loading .45s linear forwards infinite;
    }
    .vm-loading-txt {
        margin-left: 10px;
        font-size: 15px;
        color: #FFF;
        max-width: 140px;
        overflow: hidden;
        white-space: nowrap;
        text-overflow: ellipsis;
    }
}
</style>
